...
"""
    ...
    while len(cont_order) > 0:
        ...
        A_cont = list(A_cont); B_cont = list(B_cont)

        #test = np.tensordot(tensor_list[locs[0]], tensor_list[locs[1]], axes=([0], [1]))
        tensor_list.append(np.tensordot(tensor_list[locs[0]], tensor_list[locs[1]], dims=(A_cont, B_cont)))
    ...
    while len(tensor_list) > 1:
        ...
        tensor_list[-2] = np.outer(tensor_list[-2].reshape(np.prod(s1)),
                   tensor_list[-1].reshape(np.prod(s2))).reshape(onp.append(s1,s2))
    ...
    if len(connect_list[0]) > 0:
        # print(onp.argsort(-connect_list[0]))
        # print(np.transpose(tensor_list[0],list(onp.argsort(-connect_list[0]))))
        return tensor_list[0].permute(list(onp.argsort(-connect_list[0])))
        # return np.permute(tensor_list[0],list(onp.argsort(-connect_list[0])))
    else:
        #print('hello',tensor_list[0])
        return tensor_list[0]
        #return onp.asscalar(tensor_list[0])
...
def partial_trace(A, A_label):
    ...
    if num_cont > 0:
        ...
        free_dim = np.tensor(A.shape)[free_ind]

        B_label = onp.delete(A_label, cont_ind)
        cont_label = onp.unique(A_label[cont_ind])
        B = np.zeros(np.prod(free_dim))
        A = A.permute(list(onp.append(free_ind, cont_ind))).reshape(np.prod(free_dim),cont_dim,cont_dim)
        for ip in range(cont_dim):
            B = B + A[:,ip,ip]

        return B.reshape(free_dim.tolist()), B_label, cont_label
